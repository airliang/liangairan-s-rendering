...
void writeBinary(vec4* data1, vec4* data2, int N)
{
    std::ofstream fs1("results/ltc_1.data", std::ofstream::binary);
    fs1.write((const char*)data1, sizeof(vec4) * N * N);
    fs1.close();

    std::ofstream fs2("results/ltc_2.data", std::ofstream::binary);
    fs2.write((const char*)data2, sizeof(vec4) * N * N);
    fs2.close();
}
