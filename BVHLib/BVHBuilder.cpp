...
namespace BVHLib
{
	...
	BVHBuildNode* BVHBuilder::RecursiveBuild(NodeSpec& spec, int level, float progressStart, float progressEnd)
	{
		totalNodes++;
	...
	bool BVHBuilder::SplitPrimRef(const Reference& refPrim, int axis, float split, Reference& leftref, Reference& rightref)
	{
	...
	void BVHBuilder::SplitPrimRefs(const SahSplit& split, const NodeSpec& req, std::vector<Reference>& refs, int& extra_refs)
	{
	...
	BVHBuildNode* BVHBuilder::CreateLeaf(const NodeSpec& spec)
	{
		leafNodes++;
	...
	BVHBuildNode* BVHBuilder::CreateInnerNode(const Bounds3f& bounds, BVHBuildNode* left, BVHBuildNode* right)
	{
		innerNodes++;
		BVHBuildNode* node = AllocateNode();
		node->bounds = bounds;
	...
	SahSplit BVHBuilder::FindObjectSplit(const NodeSpec& spec)
	{
		SahSplit split;
		...
		//std::vector<BucketInfo> bins[3];
		//bins[0].resize(NumSpatialBins);
		//bins[1].resize(NumSpatialBins);
		//bins[2].resize(NumSpatialBins);
		...
		for (int axis = 0; axis < 3; axis++)
		{
			...
			for (int i = 0; i < nBuckets - 1; ++i)
			{
				...
				float sahTemp = m_traversalCost +
					(GetTriangleCost(leftcount) * leftBounds.SurfaceArea() + GetTriangleCost(rightcount) * rightBox.SurfaceArea()) * invSurfaceArea;
	...
	SahSplit BVHBuilder::FindSpatialSplit(const NodeSpec& spec)
	{
		// Initialize bins.
		Vector3f origin = spec.bounds.pMin;
		Vector3f binSize = (spec.bounds.pMax - origin) * (1.0f / (float)NumSpatialBins);
	...
	int BVHBuilder::FlattenBVHTree(BVHBuildNode* node, int& offset, LinearBVHNode* linearNodes)
	{
		...
		else
		{
			...
			//这里返回了offset
			linearNodes[curOffset].leftChildIdx = FlattenBVHTree(node->childrenLeft, offset, linearNodes);
			linearNodes[curOffset].rightChildIdx = FlattenBVHTree(node->childrenRight, offset, linearNodes);
		}
