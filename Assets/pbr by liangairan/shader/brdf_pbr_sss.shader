...
Shader "liangairan/pbr/pbr with sss" {
// 　　　　　　D(h) F(v,h) G(l,v,h)
//f(l,v) = ---------------------------
// 　　　　　　4(n·l)(n·v)
	Properties {
...
#include "pbrInclude.cginc"
        ...
        sampler2D _BRDFLUTTex;
            ...
            half4 frag(VSOut i) : COLOR
            {
                ...
                half3 kS = F;
                kS = half3(0.8, 0.9, 0.75);
                half3 kD = (half3(1, 1, 1) - kS) * (1.0 - _Metallic);
                //half3 directDiffuse = (albedo.rgb / PI) * kD * totalLightColor * NdL;
                half3 directDiffuse = rough_sss(lightDirection, viewDirection, h, normalDirection, _Roughness, albedo.rgb, kS) * _LightColor0;
                ...
                half3 indirectDiffuse = rough_sss(normalDirection, viewDirection, h, normalDirection, _Roughness, albedo.rgb, kS) * irradianceColor.rgb;
